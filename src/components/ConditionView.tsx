...
export function ConditionView({ conditionId, onBack, initialTier, initialWeight, autoCalculate }: ConditionViewProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Back + title */}
      <div>
        <button type="button" onClick={onBack} className="md:hidden flex items-center gap-1 text-blue-600 text-sm font-medium mb-3">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
          Back
        </button>
        {/* Title row with single flag button — context updates dynamically */}
        <div className="flex items-start justify-between gap-2">
          <h2 className="text-xl font-bold text-gray-800">{condition.name}</h2>
          <FlagButton context={{
            screen: showDoses && weightKg ? 'result' : 'details',
            condition: condition.name,
            drug: null,
            weight_kg: showDoses && weightKg ? weightKg : null,
